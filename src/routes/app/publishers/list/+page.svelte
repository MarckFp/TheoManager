...
<div class="p-4">
    <div class="card bg-base-300 rounded-box items-center h-20 grid grid-cols-3 gap-16">
        ...
        <span class="text-2xl justify-center text-center font-semibold">{$_("nav.list")}</span>
        <div class="w-10"></div>
    </div>
</div>
...
<!--{#if isMobile()}-->
    <button class="btn btn-circle float-btn btn-xl" on:click={() => (document.getElementById('create_publisher') as HTMLDialogElement)?.showModal()}>
