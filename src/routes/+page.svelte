    ...
    <div class="max-w-lg">
      ...
      {:else if stage === 3}
        ...
        <div class="grid grid-cols-1 gap-4 py-6">
          <button
            class="btn btn-accent p-10"
            onclick={() =>
              (
                document.getElementById(
                  "congregation_join"
                ) as HTMLDialogElement
              )?.showModal()}>{$_("nav.join")}</button
          >
          <button
            ...
            onclick={() =>
              ...
              )?.showModal()}>{$_("nav.create")}</button
          >
        </div>
      {/if}
      {#if stage > 1}
        <button class="btn btn-secondary" onclick={() => stage--}
          >{$_("nav.back")}</button
        >
      {/if}
      {#if stage < 3}
        <button class="btn btn-primary" onclick={() => stage++}
          >{$_("nav.next")}</button
        >
      {/if}
    </div>
...
<dialog id="congregation_create" class="modal modal-center sm:modal-middle">
  <div class="modal-box">
    ...
    <input
      ...
      placeholder="New World - North"
      required
      bind:value={congregationName}
    />

    <label class="label mt-2" for="congregation-address"
      >Congregation Address:</label
    >
    <input
      type="text"
      class="input w-full"
      ...
      placeholder="Address"
      required
      bind:value={congregationAddress}
    />
    ...
    <input
      ...
      placeholder="City"
      required
      bind:value={congregationCity}
    />

    <label class="label mt-2" for="congregation-country"
      >Congregation Country:</label
    >
    <input
      type="text"
      class="input w-full"
      ...
      placeholder="Country"
      required
      bind:value={congregationCountry}
    />

    <label class="label mt-2" for="congregation-zipcode"
      >Congregation Zipcode:</label
    >
    <input
      type="text"
      class="input w-full"
      ...
      placeholder="Zipcode"
      required
      bind:value={congregationZipcode}
    />
